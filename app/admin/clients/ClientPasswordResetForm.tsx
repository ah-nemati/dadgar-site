  ...
  return (
    <details className="dashboard-card mt-6 group">
      <summary className="cursor-pointer list-none p-4 md:p-5 flex items-center justify-between gap-3 font-bold">
        <span className="flex items-center gap-2"><KeyRound size={18} /> تعیین رمز موقت کاربر</span>
      ...
      <form action={formAction} className="border-t border-border p-4 md:p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <Input id="temporary-password" name="password" type="password" minLength={12} maxLength={128} autoComplete="new-password" required />
        </div>
        <div className="space-y-2">
          <Label htmlFor="temporary-password-confirm">تکرار رمز موقت</Label>
          <Input id="temporary-password-confirm" name="passwordConfirm" type="password" minLength={12} maxLength={128} autoComplete="new-password" required />
        </div>
