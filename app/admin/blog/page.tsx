      ...
      {posts.length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {posts.map((post) => (
            <article
              key={post.id}
              className="dashboard-card overflow-hidden flex flex-col sm:flex-row"
            >
                ...
                <div className="flex flex-wrap items-center gap-2 mb-3">
                  ...
                  {post.featured && (
                    <Badge variant="default">
                      <Star size={12} fill="currentColor" /> ویژه
                    </Badge>
                  )}
                </div>
                <h2 className="font-bold leading-7 line-clamp-2">
                  {post.title}
                </h2>
