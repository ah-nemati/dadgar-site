...
import { Award, ShieldCheck, Search, Calendar, MessageSquare, ArrowLeft, Phone, type LucideIcon } from 'lucide-react';
...
const whyUs: InfoPoint[] = [
  { icon: Award, title: 'تمرکز تخصصی', text: 'با تمرکز بر دعاوی ملکی، چک و خانواده، راهکار حقوقی متناسب با شرایط خاص شما ارائه می‌شود.' },
    ...
    <>
      ...
      <section className="bg-parchment">
        <div className="max-w-6xl mx-auto px-6 py-20">
          <div className="max-w-2xl mb-12">
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-4">در کنار شما، در تخصصی‌ترین حوزه‌های حقوقی</h2>
            <p className="text-muted-foreground leading-7">هر پرونده، ویژگی‌های حقوقی خاص خود را دارد. با تمرکز موضوعی روی هر حوزه، راهکاری متناسب با شرایط شما ارائه می‌شود.</p>
          </div>
      ...
      <section className="bg-parchment">
        <div className="max-w-6xl mx-auto px-6 py-20">
          ...
          <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-12">وکیلی که پرونده شما را پیگیری می‌کند</h2>
          {lawyers[0] && (
            <div className="bg-card border border-border rounded-sm p-8 md:p-10 flex flex-col md:flex-row items-center md:items-start gap-8 text-center md:text-right">
              <Avatar initials={lawyers[0].initials} size={112} />
              <div className="flex-1">
                <h3 className="text-xl font-bold text-foreground mb-1">{lawyers[0].name}</h3>
                <p className="text-teal font-semibold text-sm mb-4">{lawyers[0].role} — {lawyers[0].experience}</p>
                <p className="text-muted-foreground leading-7 mb-6 max-w-2xl">{lawyers[0].bio}</p>
                <Button asChild>
                  <Link href={`/lawyers/${lawyers[0].slug}`}>
                    مشاهده پروفایل کامل <ArrowLeft size={16} aria-hidden="true" />
                  </Link>
                </Button>
              </div>
            </div>
        ...
        <div className="max-w-6xl mx-auto px-6 py-20">
          <div className="flex flex-wrap items-end justify-between gap-6 mb-12">
            <div className="max-w-2xl">
              ...
              <h2 className="text-3xl md:text-4xl font-bold text-foreground">آخرین مطالب و یادداشت‌های حقوقی</h2>
            </div>
