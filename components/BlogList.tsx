      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((post) => (
            ...
            >
              {post.imageUrl ? (
                <img src={post.imageUrl} alt={post.imageAlt || post.title} className="blog-cover" loading="lazy" />
              ) : (
                <div className="blog-cover bg-gradient-to-br from-ink to-ink-2 flex items-center justify-center">
                  <span className="font-display text-parchment/70 text-lg">یادداشت حقوقی</span>
                </div>
              )}
              <div className="p-6 flex flex-col flex-1">
                <div className="flex items-center justify-between gap-3 mb-3">
                  <span className="text-xs font-semibold text-teal">{post.category}</span>
                  {post.featured && (
                    <span className="inline-flex items-center gap-1 text-[11px] text-primary">
                      <Star size={13} fill="currentColor" aria-hidden="true" /> ویژه
                    </span>
                  )}
                </div>
                <h3 className="text-base font-bold text-foreground mb-3 leading-7">{post.title}</h3>
                <p className="text-sm text-muted-foreground leading-7 mb-5 line-clamp-3">{post.excerpt}</p>
