...
export default function DashboardShell({
  ...
  return (
    <div className="dashboard-shell">
      {open && <button className="dashboard-backdrop lg:hidden" onClick={() => setOpen(false)} aria-label="بستن منو" />}
      ...
      <aside className="dashboard-sidebar" data-open={open}>
        <div className="h-[4.5rem] px-5 border-b border-white/10 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-3" onClick={() => setOpen(false)}>
            <Seal size={36} />
            <div>
              <p className="font-display text-sm text-parchment">{panelTitle}</p>
              <p className="text-[11px] text-parchment/55 mt-1">دفتر خدمات حقوقی</p>
            </div>
          </Link>
          <button className="header-icon-button lg:hidden" onClick={() => setOpen(false)} aria-label="بستن منو">
            <X size={21} />
          </button>
        </div>
      ...
      <div className="dashboard-main">
        <header className="dashboard-topbar">
          <button className="lg:hidden inline-flex items-center justify-center size-10 rounded-full hover:bg-muted" onClick={() => setOpen(true)} aria-label="باز کردن منوی پنل">
            <Menu size={22} />
          </button>

          <div className="mr-auto lg:mr-0 min-w-0">
            <p className="text-xs text-muted-foreground">{account.role === 'admin' ? 'مدیر سایت' : 'حساب موکل'}</p>
            <p className="text-sm font-bold truncate">{account.fullName || account.email}</p>
          </div>
