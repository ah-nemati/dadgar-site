...
async function runClientDeepTest() {
  ...
  const [clientUser] =
    await db`select id, email, role, name, phone from users where email = 'bbb@bbb.bbb'`;
  const [adminUser] =
    await db`select id, email, role from users where email = 'amirhoseinnemati79@gmail.com'`;
  ...
  console.log(
    "1. Setting up sample data for Client (Case, Update, Document, Support Thread)...",
  );

  // Insert or get test case
  let [testCase] =
    await db`select id, title, case_number from client_cases where client_id = ${clientUser.id} limit 1`;
  if (!testCase) {
    [testCase] = await db`
  ...
  let [caseUpdate] =
    await db`select id, title from case_updates where case_id = ${testCase.id} limit 1`;
  if (!caseUpdate) {
    [caseUpdate] = await db`
      ...
      values (
        ...
        'استعلام وضعیت پلاک ثبتی از اداره ثبت اسناد   صادر گردید و لایحه دفاعیه جهت جلسه اول رسیدگی آماده و ارسال شد.',
        now() - interval '2 days'
      )
      returning id, title
    `;
  ...
  let [caseDoc] =
    await db`select id, title from client_documents where case_id = ${testCase.id} limit 1`;
  if (!caseDoc) {
    [caseDoc] = await db`
  ...
  let [thread] =
    await db`select id, subject from support_threads where client_id = ${clientUser.id} limit 1`;
  if (!thread) {
    [thread] = await db`
  ...
  const existingMsgs =
    await db`select id from support_messages where thread_id = ${thread.id}`;
  ...
  const clientToken = await generateSessionToken(
    clientUser.id,
    clientUser.role,
  );
  ...
  for (const item of portalUrls) {
    ...
    const hasLogoutBtn =
      html.includes("خروج") || html.includes("در حال خروج...");
    ...
    if (item.url === "/portal") {
      ...
      specificCheck =
        html.includes("نوبت مشاوره") && html.includes("زمان نوبت:");
      ...
      specificCheck =
        html.includes("روند پرونده") && html.includes("اسناد قابل دریافت");
      ...
      specificCheck =
        html.includes("دفتر وکالت") &&
        html.includes("شما") &&
        html.includes("پاسخ جدید");
      specificDetail = "Message chat conversation & reply form";
    } else if (item.url === "/portal/profile") {
      specificCheck =
        html.includes("اطلاعات حساب کاربری") &&
        html.includes("تغییر رمز عبور") &&
        html.includes(clientUser.email);
      specificDetail = "Profile form with email & change password form";
    }

    if (is200 && hasLogoutBtn && !hasNaN && !hasInvalidDate && specificCheck) {
      console.log(
        `✅ PASS: ${item.url} -> [${item.label}] -> Content Verified (${specificDetail})`,
      );
    } else {
      console.error(
        `❌ FAIL: ${item.url} -> [${item.label}] (200: ${is200}, logout: ${hasLogoutBtn}, NaN: ${hasNaN}, invalidDate: ${hasInvalidDate}, specific: ${specificCheck})`,
      );
      testPassed = false;
    }
  }
  ...
  console.log(
    `   -> Client accessing /admin -> Status: ${adminRes.status} (Correctly denied/redirected)`,
  );
  ...
  console.log(
    `   -> Client accessing /admin/clients -> Status: ${adminClientsRes.status} (Correctly denied/redirected)`,
  );
